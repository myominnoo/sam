...
More items available. Use offset: ${n.pagination.offset+n.pagination.limit} to see the next page.`),p}function w(n){if(!n?.length)return null;let t=_a(n);return t.length===0?null:`Unknown type${t.length===1?"":"s"}: ${t.join(", ")}. Valid types: ${Za.join(", ")}.`}function $(n){if(!n.errors?.length)return "";let t=n.errors.map(r=>`- ${r.registry}: ${r.message}`);return `
