...
export const ManageData = ({
  ...
  const [editProjectStaffIds, setEditProjectStaffIds] = useState<number[]>([]);
  ...
  const startEditStaff = (staff: Staff) => {
    ...
    setEditStaffRole(staff.designation);
  ...
  const startEditProject = (proj: Project) => {
    ...
    const currentStaffIds = assignments
      ...
      .map((a) => a.staffId);
    setEditProjectStaffIds(currentStaffIds);
  };

  const handleToggleProjectStaff = (staffId: number) => {
    setEditProjectStaffIds((prev) =>
        ...
        <div className="p-6 space-y-6">
          ...
          >
            ...
            <input
              ...
              className="w-20 px-3 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            <input
              type="number"
              step="0.1"
                        ...
                        <>
                          ...
                          <td className="p-2">
                            <input
                              ...
                              className="w-16 px-2 py-1 border rounded text-xs focus:ring-2 focus:ring-indigo-500"
                            />
                          </td>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-slate-800">Projects Directory</h2>
          </div>
                          ...
                          <td className="p-2">
                            <div className="space-y-1.5">
                              <select
                                onChange={(e) => {
                                  const val = Number(e.target.value);
                                  if (val) handleToggleProjectStaff(val);
                                  e.target.value = "";
                                }}
                                className="w-full bg-white border border-slate-300 rounded px-2 py-1 text-xs font-medium text-slate-700 focus:outline-none"
                              >
                                ...
                                {staffMembers.map((s) => (
                                  <option
                                    key={s.id}
                                    value={s.id}
                                    disabled={editProjectStaffIds.includes(
                                      s.id!,
                                    )}
                                  >
                                    {s.name}
